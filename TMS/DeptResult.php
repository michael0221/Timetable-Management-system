  	...
  	if($_POST['B1'])
  	{
  		...
  		$DeptName = $_POST['T2'];
   		...
   		//[1]update Department Name
   			...
   			$res=mysqli_query($mysqli,"update Departments set DeptName='$DeptName' where UniversityCode='$uncode' and CollegeCode='$CollegeCode' and DeptNo='$DeptNo' and AcadDegreeId='$AcadProgId'");
   		
   		if($res)
   		{
   			//[2]update AcadProg Name or NoOfSem

			$res=mysqli_query($mysqli,"update AcadDegree set AcadDegreeName='$AcadProg', NoOfSemester='$noOfSem', AcadProgType='$ProgType' where AcadDegreeId='$AcadProgId'");
	...
	if($_POST['B2'])
  	{
  		...
  		$res=mysqli_query($mysqli,"delete from DeptandSem  where UniversityCode='$uncode' and CollegeCode='$CollegeCode' and DeptNo='$DeptNo' and AcadDegreeId='$AcadProgId'");
  		  	
  		if($res)
  		{
  		  	$res=mysqli_query($mysqli,"delete from Departments where UniversityCode='$uncode' and CollegeCode='$CollegeCode' and DeptNo='$DeptNo' and AcadDegreeId='$AcadProgId'");
  		  	
  		  if($res)
  		  {
  		  	  	$res=mysqli_query($mysqli,"delete from AcadDegree where AcadDegreeId='$AcadProgId'");
   ...
   if (!filled_out($_POST))
	...
	if($flag==true)
	{
		...
		//echo("</br>Insert AcadProg:</br>Prog Id=".$AcadId."</br>"."name=".$AcadProg."</br>NoofSem=".$noOfSem."</br>");
			...
			$res2=mysqli_query($mysqli,"insert into acaddegree( `AcadDegreeId` , `AcadDegreeName` , `NoOfSemester`, `AcadProgType`) VALUES ('$AcadId','$AcadProg','$noOfSem','$ProgType')");
		...
		if(	$res2)
		{
			...
			if($result3)
			{
				...
				$result333 = mysqli_query($mysqli,$sql333);
